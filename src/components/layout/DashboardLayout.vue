...
// No sidebar state needed for this layout
</script>
        ...
        <div class="flex justify-between h-24 items-center">
          <div class="flex items-center gap-3 sm:gap-4">
            <div class="p-1.5 sm:p-2 bg-white/5 rounded-lg border border-white/10 backdrop-blur-sm shadow-inner">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 sm:h-6 sm:w-6 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="hidden sm:flex items-center gap-6">
            <div class="flex items-center gap-3 text-right">
              ...
              <span class="text-lg font-semibold text-white">{{ new Date().toLocaleDateString('th-TH', { dateStyle: 'long' }) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <footer class="bg-white border-t border-border mt-auto">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="flex justify-center items-center text-sm text-text-muted">
          <p>© {{ new Date().getFullYear() }} ฝ่ายเภสัชกรรม โรงพยาบาลสระโบสถ์.</p>
        </div>
      </div>
    </footer>
  </div>
